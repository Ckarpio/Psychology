 ...
 * Поддерживает все форматы ссылок Pinterest
 * @param {string} url - ссылка на пин или изображение Pinterest
 * @returns {string|null} - прямая ссылка на изображение или null
 */
function extractPinterestImageUrl(url) {
    ...
    // Паттерн 1: pin.it/XXXXX (короткие ссылки)
    if (url.includes('pin.it/')) {
        // Для коротких ссылок нужно получить ID из перенаправления
        // В браузере мы не можем следовать редиректу напрямую, поэтому используем прокси или просто возвращаем как есть
        console.warn('⚠️ Короткая ссылка pin.it, может потребоваться прямой URL изображения');
        // Пробуем получить через API, но в браузере это сложно
        // Возвращаем null, пользователь должен вставить прямую ссылку
        return null;
    }
    
    // Паттерн 2: pinterest.com/pin/1234567890/
    ...
    if (pinMatch) {
        ...
        // Формируем прямую ссылку на изображение (разные размеры)
        imageUrl = `https://i.pinimg.com/originals/${getPinPathFromId(pinId)}`;
        console.log('✅ Сгенерирована прямая ссылка:', imageUrl);
        return imageUrl;
    }
    
    // Паттерн 3: pinterest.com/pin/1234567890/embed/
    const embedMatch = url.match(/pin\/(\d+)\/embed/i);
    if (embedMatch) {
        const pinId = embedMatch[1];
        imageUrl = `https://i.pinimg.com/originals/${getPinPathFromId(pinId)}`;
        return imageUrl;
    }
    
    // Паттерн 4: pinterest.com/username/pin/1234567890/
    const usernameMatch = url.match(/\/([^\/]+)\/pin\/(\d+)/i);
    if (usernameMatch) {
        const pinId = usernameMatch[2];
        imageUrl = `https://i.pinimg.com/originals/${getPinPathFromId(pinId)}`;
        return imageUrl;
    }
    
    // Если ничего не подошло, возвращаем null
    console.warn('❌ Не удалось распознать ссылку Pinterest');
    return null;
}

/**
 * Вспомогательная функция для формирования пути к изображению по ID пина
 * @param {string} pinId - ID пина
 * @returns {string} - путь к изображению
 */
function getPinPathFromId(pinId) {
    // Pinterest использует хеширование ID для формирования пути
    // Это упрощенная версия, для реального использования может потребоваться другой подход
    const hash = Math.abs(parseInt(pinId) % 1000).toString().padStart(3, '0');
    return `${hash}/${pinId}.jpg`;
}

/**
 * Функция для загрузки и проверки изображения
 * @param {string} url - URL изображения
 * @returns {Promise<string|null>} - URL, если изображение доступно
 */
async function loadImage(url) {
    if (!url) return null;
    
    // Пытаемся получить изображение через разные размеры
    const sizeVariants = [
        url, // оригинал
        url.replace('/originals/', '/736x/'), // средний размер
        url.replace('/originals/', '/564x/'), // другой размер
        url.replace('/originals/', '/1200x/') // большой размер
    ];
    
    for (const variantUrl of sizeVariants) {
        try {
            console.log('🔄 Пробуем загрузить:', variantUrl);
            const response = await fetch(variantUrl, { 
                method: 'HEAD',
                mode: 'no-cors' // для Pinterest может потребоваться
            }).catch(() => null);
            
            // Для Pinterest сложно проверить, поэтому просто возвращаем URL
            if (variantUrl.includes('i.pinimg.com')) {
                return variantUrl;
            }
            ...
            console.log('⚠️ Ошибка загрузки варианта:', variantUrl);
        }
    }
...
/**
 * Рендеринг фотографий с поддержкой Pinterest ссылок
 */
async function renderImages(items) {
    ...
    for (const item of items) {
        // Получаем ссылку из поля "изображения"
        const originalUrl = item.изображения;
        ...
        const isPinterest = finalImageUrl.includes('i.pinimg.com');
                ...
                <div style="position: relative; width: 100%; height: 220px; background: #f5f5f5; overflow: hidden;">
                    <img src="${finalImageUrl}" 
                         ...
                         onerror="this.onerror=null; 
                                  this.parentElement.innerHTML='<div style=\'width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#f0f0f0;color:#999;\'><span style=\'font-size:48px;\'>🖼️</span><span style=\'margin-top:10px;\'>Фото не доступно</span><span style=\'font-size:12px; margin-top:5px;\'>${originalUrl.substring(0, 50)}...</span></div>';">
                </div>
                ...
                <div class="music-item vk-music" style="margin-bottom: 25px; padding: 30px; background: linear-gradient(135deg, #0077FF08 0%, #0077FF15 100%); border: 1px solid #0077FF30; border-radius: 24px; box-shadow: 0 10px 25px rgba(0,119,255,0.1);">
                    <div style="display: flex; align-items: center; gap: 25px; flex-wrap: wrap;">
                        ...
                        <div style="flex: 2; min-width: 300px;">
                            ...
                            <h3 style="margin: 0 0 8px 0; color: #1A1A1A; font-size: 28px; font-weight: 700;">${trackTitle}</h3>
                            
                            ${artist ? `<div style="margin: 0 0 20px 0; color: #0077FF; font-size: 22px; font-weight: 600;">${artist}</div>` : ''}
                            
                            <a href="${audioUrl}" target="_blank" 
                               style="display: inline-flex; align-items: center; gap: 12px; background: #0077FF; color: white; text-decoration: none; padding: 16px 32px; border-radius: 50px; font-weight: 600;"
