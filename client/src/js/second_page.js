...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!emotionCode) {
        // На главной странице показываем все эмоции
        if (window.location.pathname.includes('index.html') || window.location.pathname === '/') {
            await loadAllEmotions();
        } else {
            displayError('Эмоция не выбрана');
        }
        return;
...
// Загрузка всех эмоций для главной страницы
async function loadAllEmotions() {
    try {
        const response = await fetch(`${API_URL}/api/emotions`);
        const emotions = await response.json();
        
        // Разделяем эмоции на позитивные и негативные
        const positiveEmotions = emotions.filter(e => e.type === 'positive');
        const negativeEmotions = emotions.filter(e => e.type === 'negative');
        
        displayEmotionsGrid(positiveEmotions, negativeEmotions);
    } catch (error) {
        console.error('Ошибка загрузки эмоций:', error);
        displayError('Не удалось загрузить список эмоций');
    }
}

// Отображение сетки эмоций
function displayEmotionsGrid(positiveEmotions, negativeEmotions) {
    const container = document.querySelector('.container');
    if (!container) return;
    
    let html = `
        <h1>Эмоциональная поддержка</h1>
        <p class="subtitle">Выберите эмоцию, которая вас сейчас беспокоит, или найдите ресурс для позитивного состояния</p>
        
        <div class="emotions-section">
            <h2 class="section-title positive-title">🌟 Позитивные эмоции</h2>
            <div class="emotions-grid positive-grid">
    `;
    
    // Добавляем позитивные эмоции (ограничиваем до 6 для двух строк по 3)
    const displayPositive = positiveEmotions.slice(0, 6);
    displayPositive.forEach(emotion => {
        html += `
            <div class="emotion-card positive" onclick="window.location.href='emotion.html?emotion=${emotion.code}'">
                <div class="emotion-emoji">${emotion.emoji || '😊'}</div>
                <div class="emotion-name">${emotion.label || emotion.code}</div>
                <div class="emotion-desc">${emotion.short_description || ''}</div>
            </div>
        `;
    });
    
    // Если позитивных эмоций меньше 6, добавляем заглушки
    for (let i = displayPositive.length; i < 6; i++) {
        html += `
            <div class="emotion-card placeholder">
                <div class="emotion-emoji">✨</div>
                <div class="emotion-name">Скоро появится</div>
                <div class="emotion-desc">Новые эмоции добавляются</div>
            </div>
        `;
    }
    ...
    html += `
        ...
        <div class="emotions-section">
            <h2 class="section-title negative-title">💭 Негативные эмоции</h2>
            <div class="emotions-grid negative-grid">
    `;
    
    // Добавляем негативные эмоции
    negativeEmotions.forEach(emotion => {
        html += `
            <div class="emotion-card negative" onclick="window.location.href='emotion.html?emotion=${emotion.code}'">
                <div class="emotion-emoji">${emotion.emoji || '😔'}</div>
                <div class="emotion-name">${emotion.label || emotion.code}</div>
                <div class="emotion-desc">${emotion.short_description || ''}</div>
            </div>
...
// Добавление глобальных стилей для главной страницы
function addGlobalStyles() {
    ...
    style.textContent = `
        ...
        .emotion-card.positive:hover {
            border-color: #4CAF50;
            background: linear-gradient(135deg, #fff 0%, #f0f9f0 100%);
        }
        
        .emotion-card.negative:hover {
            border-color: #f44336;
        ...
        .emotion-card.placeholder:hover {
            transform: none;
